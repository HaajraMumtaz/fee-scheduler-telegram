...
class StudentSyncService:
    ...
    def sync_from_sheet(self, rows: list[dict]):
        ...
        for row in rows:
            ...
            external_id = data.get("student_id")
            if not external_id:
                result["skipped"] += 1
                continue
            ...
            if existing:
                changed = False
                for field in ["name", "fee_due_day", "poc_name", "poc_phone"]:
...
class TeacherSyncService:
    ...
    def sync_from_sheet(self, rows: list[dict]):
        ...
        for row in rows:
            ...
            existing = self.teacher_repo.get_by_id(teacher_id)
            ...
            if existing:
                ...
                # status is present but not used yet
